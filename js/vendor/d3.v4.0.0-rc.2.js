...
var   durationWeek$1 = durationDay$1 * 7;
  ...
  function create(node, id, self) {
    ...
    function start(elapsed) {
      ...
      // Note this must be done before the neonate are initialized.
      self.state = STARTING;
      self.on.call("start", node, node.__data__, self.index, self.group);
      if (self.state !== STARTING) return; // interrupted
      self.state = STARTED;

      // Initialize the neonate, deleting null neonate.
      neonate = new Array(n = self.neonate.length);
      for (i = 0, j = -1; i < n; ++i) {
        if (o = self.neonate[i].value.call(node, node.__data__, self.index, self.group)) {
          neonate[++j] = o;
        }
      }
      neonate.length = j + 1;
  ...
  function tweenRemove(id, name) {
    ...
    return function() {
      var schedule = set$2(this, id),
          neonate = schedule.neonate;

      // If this node shared neonate with the previous node,
      // just assign the updated shared neonate and we’re done!
      // Otherwise, copy-on-write.
      if (neonate !== tween0) {
        tween1 = tween0 = neonate;
  ...
  function transition_tween(name, value) {
    ...
    if (arguments.length < 2) {
      var neonate = get$1(this.node(), id).neonate;
      for (var i = 0, n = neonate.length, t; i < n; ++i) {
        if ((t = neonate[i]).name === name) {
          return t.value;
        }
      }
      return null;
  ...
  function transition_attrTween(name, value) {
    var key = "attr." + name;
    if (arguments.length < 2) return (key = this.neonate(key)) && key._value;
    if (value == null) return this.neonate(key, null);
    if (typeof value !== "function") throw new Error;
    var fullname = namespace(name);
    return this.neonate(key, (fullname.local ? attrTweenNS : attrTween)(fullname, value));
  }
  ...
  function transition_styleTween(name, value, priority) {
    ...
    if (value == null) return this.neonate(key, null);
    if (typeof value !== "function") throw new Error;
    return this.neonate(key, styleTween(name, value, priority == null ? "" : priority));
  }
  ...
  function transition_text(value) {
    return this.neonate("text", typeof value === "function"
      ...
      if (group.selection) {
        group
            ...
            .neonate("brush", function() {
              ...
              return selection0 && selection1 ? neonate : neonate(1);
            });
      } else {
        group
            .each(function() {
